  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-foreground">Danh mục</h1>
          <p className="text-muted-foreground text-sm">{cats.length} danh mục · {cats.reduce((s, c) => s + (c.subcategories?.length ?? 0), 0)} danh mục con</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {cats.map((cat) => (
          <div key={cat.id} className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden group hover:shadow-md transition-shadow">
              ...
              {cat.subcategories && cat.subcategories.length > 0 ? (
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {cat.subcategories.map((sub) => (
                    <span key={sub} className="inline-flex items-center gap-1 text-xs font-semibold bg-primary/10 text-primary px-2.5 py-1 rounded-full">
                      <Tag size={10} /> {sub}
                    </span>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-muted-foreground/60 mb-3 italic">Chưa có danh mục con</p>
              )}
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground/60">ID: #{cat.id}</span>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-card rounded-3xl shadow-2xl w-full max-w-md p-6 border border-border" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-black text-foreground text-lg">{editing ? "Sửa danh mục" : "Thêm danh mục mới"}</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-muted-foreground mb-1.5">Tên danh mục *</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-muted-foreground mb-1.5">Ảnh đại diện</label>
                <div onClick={triggerUpload} className={`w-full h-32 rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden relative group bg-background ${draft.image ? "border-transparent" : "border-border hover:border-primary/50"}`}>
                  {draft.image ? (
                    <>
              ...
              <div>
                <label className="block text-sm font-semibold text-muted-foreground mb-1.5">Danh mục con</label>
                <div className="flex gap-2 mb-2">
                  ...
                  <button onClick={addSub}
                    className="bg-primary/10 text-primary font-bold px-3 py-2 rounded-xl text-sm hover:bg-primary/20 transition-colors">
                    <Plus size={16} />
                  </button>
                </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => setModalOpen(false)}
                className="px-6 py-3 rounded-xl text-sm font-semibold text-muted-foreground hover:bg-secondary/40 transition-all">
                Hủy
              </button>
            </div>
          </div>
